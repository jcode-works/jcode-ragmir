...
export const AGENT_HELPER_CONFIG_FILENAMES = {
...
async function writeAgentMcpHelper(agent, input) {
    switch (agent) {
        ...
        case "kimi":
        case "cline":
            await writeFile(agent === "kimi" ? input.kimiConfigPath : input.clineConfigPath, `${JSON.stringify(mcpConfig(input.cwd, input.serveCommand, { [RAGMIR_PROJECT_ROOT_ENV]: input.cwd }, input.mcpServerName), null, 2)}\n`, "utf8");
    }
}
...
\`\`\``);
    ...
    sections.push("For other MCP clients that cannot set a working directory, launch the server with `RAGMIR_PROJECT_ROOT=/absolute/path/to/repository`.");
