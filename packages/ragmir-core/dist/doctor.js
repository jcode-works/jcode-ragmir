...
function nextActions(input) {
    ...
    if (input.supportedFiles === 0) {
        if (input.skippedFiles > 0) {
            steps.push("Ragmir found files, but none are currently indexable. Run `ragmir audit --unsupported` to inspect skipped files.");
        }
        else {
            steps.push('Add supported files under .ragmir/raw/ or list extra source paths in the "sources" array of .ragmir/config.json.');
        }
        return steps;
    }
    ...
    if (steps.length === 0) {
        if (input.unsupportedFiles > 0) {
            steps.push("Run `ragmir audit --unsupported` to inspect files skipped because their type is not supported.");
        }
        ...
        if (input.agentKitInstalled) {
            steps.push("Run `ragmir install-agent --agents claude` or another targeted agent list for native skill discovery.");
            steps.push("Wire the matching MCP helper from .ragmir/ when the agent should call Ragmir tools directly.");
        }
        else {
...
function isAgentKitInstalled(projectRoot) {
    ...
    const agentHelpers = Object.values(AGENT_HELPER_CONFIG_FILENAMES).map((filename) => path.join(ragmirDir, filename));
