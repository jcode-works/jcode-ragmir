...
function applyEnv(config) {
    return {
        ...
        accessLogPath: readStringEnv("RAGMIR_ACCESS_LOG_PATH", "KB_ACCESS_LOG_PATH", config.accessLogPath),
        embeddingProvider: readEmbeddingProviderEnv("RAGMIR_EMBEDDING_PROVIDER", "KB_EMBEDDING_PROVIDER", config.embeddingProvider),
        embeddingModel: readStringEnv("RAGMIR_EMBEDDING_MODEL", "KB_EMBEDDING_MODEL", config.embeddingModel),
        embeddingModelPath: readStringEnv("RAGMIR_EMBEDDING_MODEL_PATH", "KB_EMBEDDING_MODEL_PATH", config.embeddingModelPath),
        transformersAllowRemoteModels: readBooleanEnv("RAGMIR_TRANSFORMERS_ALLOW_REMOTE_MODELS", "KB_TRANSFORMERS_ALLOW_REMOTE_MODELS", config.transformersAllowRemoteModels),
        redaction: {
            ...config.redaction,
            enabled: readBooleanEnv("RAGMIR_REDACTION_ENABLED", "KB_REDACTION_ENABLED", config.redaction.enabled),
            builtIn: readBooleanEnv("RAGMIR_REDACTION_BUILT_IN", "KB_REDACTION_BUILT_IN", config.redaction.builtIn),
        },
        ...
        chunkOverlap: readNonNegativeIntEnv("RAGMIR_CHUNK_OVERLAP", "KB_CHUNK_OVERLAP", config.chunkOverlap),
        maxFileBytes: readPositiveIntEnv("RAGMIR_MAX_FILE_BYTES", "KB_MAX_FILE_BYTES", config.maxFileBytes),
        ingestConcurrency: readPositiveIntEnv("RAGMIR_INGEST_CONCURRENCY", "KB_INGEST_CONCURRENCY", config.ingestConcurrency),
        embeddingBatchSize: readPositiveIntEnv("RAGMIR_EMBEDDING_BATCH_SIZE", "KB_EMBEDDING_BATCH_SIZE", config.embeddingBatchSize),
        includeExtensions: readExtensionsEnv("RAGMIR_INCLUDE_EXTENSIONS", "KB_INCLUDE_EXTENSIONS", config.includeExtensions),
        pdfOcrCommand: readJsonStringArrayEnv("RAGMIR_PDF_OCR_COMMAND", "KB_PDF_OCR_COMMAND", config.pdfOcrCommand),
        pdfOcrTimeoutMs: readPositiveIntEnv("RAGMIR_PDF_OCR_TIMEOUT_MS", "KB_PDF_OCR_TIMEOUT_MS", config.pdfOcrTimeoutMs),
        imageOcrCommand: readJsonStringArrayEnv("RAGMIR_IMAGE_OCR_COMMAND", "KB_IMAGE_OCR_COMMAND", config.imageOcrCommand),
        imageOcrTimeoutMs: readPositiveIntEnv("RAGMIR_IMAGE_OCR_TIMEOUT_MS", "KB_IMAGE_OCR_TIMEOUT_MS", config.imageOcrTimeoutMs),
        legacyWordCommand: readJsonStringArrayEnv("RAGMIR_LEGACY_WORD_COMMAND", "KB_LEGACY_WORD_COMMAND", config.legacyWordCommand),
        legacyWordTimeoutMs: readPositiveIntEnv("RAGMIR_LEGACY_WORD_TIMEOUT_MS", "KB_LEGACY_WORD_TIMEOUT_MS", config.legacyWordTimeoutMs),
    };
}
