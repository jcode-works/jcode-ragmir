...
describe("previewChunks", () => {
  it("should preview redacted structured chunks without writing an index", async () => {
    ...
    expect(report.files[0]?.chunks[0]?.citation).not.toContain(":L")
    expect(report.files[0]?.chunks[0]?.lineStart).toBeNull()
    expect(report.files[0]?.chunks[0]?.lineEnd).toBeNull()
