...
export function getLexicalScanWarning(config: Config, chunkCount: number): string | null {
  ...
  return `Lexical search scans only the first ${config.hybridTextScanLimit} of ${chunkCount} chunks. Raise \`hybridTextScanLimit\` in .ragmir/config.json for better keyword recall.`
}
