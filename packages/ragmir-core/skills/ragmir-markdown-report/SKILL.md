---
name: ragmir-markdown-report
description: >-
  Create a cited Markdown report from a Ragmir local knowledge base. Use when the user asks for a
  report, dossier, audit memo, planning note, evidence summary, or Markdown deliverable grounded in
  private repository documents indexed by Ragmir.
---

# Ragmir Markdown Report

Use this skill to write a Markdown report from local Ragmir evidence without leaking raw documents.
...
- Prefer writing reports under `.ragmir/reports/`, which is ignored by Git when Ragmir is installed.
...
```bash
mkdir -p .ragmir/reports
```

Write the report to:

```plain text
.ragmir/reports/<subject-kebab>.md
```

If the user explicitly wants a tracked report, create a sanitized file outside `.ragmir/` and state
