...
const env = {
  ...
  RAGMIR_LICENSE_PRIVATE_KEY_JWK: JSON.stringify(privateKeyJwk),
  RAGMIR_LICENSE_MAJOR_VERSION: "0",
  RAGMIR_LICENSE_DOWNLOAD_URL: "https://downloads.example.invalid/ragmir",
  RAGMIR_LICENSE_RECORDS: memoryLicenseStore(),
}
...
async function smokeOrderLicense(publicKeyJwk) {
  ...
  assertEqual(output.downloadUrl, "https://downloads.example.invalid/ragmir", "order downloadUrl")

  const payload = await verifyLicenseKey(output.licenseKey, publicKeyJwk)
  assertEqual(payload.product, "ragmir-desktop", "payload product")
  ...
  assertEqual(env.RAGMIR_LICENSE_RECORDS.size(), 1, "stored order record count")
}

async function smokeInvalidSignature() {
  const response = await handleLicenseWebhook(
    new Request("https://ragmir-license-webhook.test/lemonsqueezy", {
      method: "POST",
...
async function signedRequest(body) {
  return new Request("https://ragmir-license-webhook.test/lemonsqueezy", {
    method: "POST",
    headers: {
...
async function verifyLicenseKey(licenseKey, publicKeyJwk) {
  ...
  if (parts.length !== 3 || parts[0] !== "RAGMIR1") {
    throw new Error("license key must use RAGMIR1.payload.signature format")
  }
  ...
  return {
    ...
    data: {
      ...
      attributes: {
        ...
        product_name: "Ragmir Desktop",
        variant_name: "Ragmir Desktop Solo",
      },
    },
  }
}
  ...
  return {
    ...
    data: {
      ...
      attributes: {
        ...
        user_email: "buyer@example.test",
        product_name: "Ragmir Desktop",
        variant_name: "Ragmir Desktop Team",
      },
    },
  }
}
