const PRODUCT_ID = "ragmir-desktop"
const LICENSE_FORMAT = "RAGMIR1"
...
export type LicenseWebhookEnv = {
  ...
  RAGMIR_LICENSE_PRIVATE_KEY_JWK?: string
  RAGMIR_LICENSE_MAJOR_VERSION?: string
  RAGMIR_LICENSE_UPDATES_YEARS?: string
  RAGMIR_LICENSE_DOWNLOAD_URL?: string
  RAGMIR_LICENSE_RECORDS?: LicenseRecordStore
}
...
export async function handleLicenseWebhook(
  ...
  try {
    ...
    const store = requiredStore(env.RAGMIR_LICENSE_RECORDS)
    ...
    const privateKeyJwk = JSON.parse(
      requiredEnv(env.RAGMIR_LICENSE_PRIVATE_KEY_JWK, "RAGMIR_LICENSE_PRIVATE_KEY_JWK"),
    )
    ...
    const body: WebhookResponseBody = {
      ...
      downloadUrl: env.RAGMIR_LICENSE_DOWNLOAD_URL ?? null,
      licenseKey,
    }
    await storeBody(store, body)
    return jsonResponse(body)
...
function orderLicensePayload(
  ...
  const updateYears = positiveInteger(env.RAGMIR_LICENSE_UPDATES_YEARS, DEFAULT_UPDATE_YEARS)
  return {
    product: PRODUCT_ID,
    ...
    majorVersion: positiveInteger(env.RAGMIR_LICENSE_MAJOR_VERSION, DEFAULT_MAJOR_VERSION),
    issuedAt,
...
function inferTier(attributes: JsonRecord): LicensePayload["tier"] {
  ...
  const explicitTier = stringValue(customData.ragmir_tier)
...
function requiredStore(value: LicenseRecordStore | undefined) {
  if (!value) {
    throw new Error("RAGMIR_LICENSE_RECORDS KV binding is required.")
  }
  return value
}
