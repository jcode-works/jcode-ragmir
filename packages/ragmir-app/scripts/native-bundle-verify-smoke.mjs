...
const tempDir = await mkdtemp(join(tmpdir(), "ragmir-native-bundle-verify-smoke-"))
...
try {
  ...
  await writeFile(join(incompleteLinuxDir, "Ragmir_0.0.0.AppImage"), "synthetic appimage\n", "utf8")
...
async function writeTargetArtifacts(artifactsDir, target) {
  if (target === "linux") {
    await writeFile(join(artifactsDir, "Ragmir_0.0.0.AppImage"), "synthetic appimage\n", "utf8")
    await writeFile(join(artifactsDir, "ragmir_0.0.0_amd64.deb"), "synthetic deb\n", "utf8")
    return
  }
  if (target === "macos") {
    await mkdir(join(artifactsDir, "Ragmir.app", "Contents"), { recursive: true })
    await writeFile(join(artifactsDir, "Ragmir_0.0.0.dmg"), "synthetic dmg\n", "utf8")
    await writeFile(
      join(artifactsDir, "Ragmir.app", "Contents", "Info.plist"),
      "synthetic plist\n",
      "utf8",
    )
    return
  ...
  if (target === "windows") {
    await writeFile(join(artifactsDir, "Ragmir_0.0.0_x64-setup.exe"), "synthetic exe\n", "utf8")
    await writeFile(join(artifactsDir, "Ragmir_0.0.0_x64_en-US.msi"), "synthetic msi\n", "utf8")
    return
  }
  await writeFile(join(artifactsDir, "ragmir-universal-release.apk"), "synthetic apk\n", "utf8")
}
