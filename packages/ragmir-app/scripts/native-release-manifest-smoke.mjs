...
const tempDir = await mkdtemp(join(tmpdir(), "ragmir-native-release-manifest-smoke-"))
const artifactsDir = join(tempDir, "bundle")
const manifestPath = join(artifactsDir, "ragmir-app-release.json")
...
try {
  ...
  assertFails(
    [
      ...
      "linux",
      "--base-url",
      "http://downloads.example.invalid/ragmir/linux",
    ],
  ...
  const result = runNode([
    ...
    manifestPath,
    "--base-url",
    "https://downloads.example.invalid/ragmir/linux",
    "--generated-at",
  ...
  assertEqual(manifest.product, "Ragmir", "product")
  assertEqual(manifest.packageName, "@jcode.labs/ragmir-app", "packageName")
  ...
  for (const entry of manifest.files) {
    ...
    assertEqual(
      entry.downloadUrl,
      `https://downloads.example.invalid/ragmir/linux/${entry.file}`,
