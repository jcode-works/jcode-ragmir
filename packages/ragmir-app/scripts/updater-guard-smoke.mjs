...
const tempDir = await mkdtemp(join(tmpdir(), "ragmir-updater-guard-smoke-"))
...
try {
  ...
  await writeJson(readyConfig, {
    ...
    plugins: {
      updater: {
        pubkey: "ragmir-updater-public-key-for-smoke-test-0001",
        endpoints: ["https://updates.example.invalid/ragmir/latest.json"],
      },
    },
  })
