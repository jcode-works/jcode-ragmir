...
const manifestPath = path.resolve(
  args.manifest ?? path.join(artifactsDir, "ragmir-app-release.json"),
)
...
if (args.json) {
  ...
  console.log(`Ragmir native bundle verification: ${report.ok ? "ok" : "failed"}`)
...
async function verifyBundle({ artifactsDir, checksumEntries, files, manifest, target }) {
  ...
  const artifactFiles = files.filter(
    (file) => file !== "SHA256SUMS" && file !== "ragmir-app-release.json",
  )
  const checks = [
    {
      label: "target",
