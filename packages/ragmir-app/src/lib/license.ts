const LICENSE_STORAGE_KEY = "ragmir.licenseKey.v1"
...
function configuredPublicKey(): PublicLicenseKey | null {
  const raw = import.meta.env.VITE_RAGMIR_LICENSE_PUBLIC_KEY_JWK
  if (!raw) {
    return null
  }
...
function licensePayloadCheck(value: unknown): LicensePayloadCheck {
  ...
  if (value.product !== PRODUCT_ID)
    return { ok: false, message: "License product is not Ragmir Desktop." }
