const PROJECT_REGISTRY_KEY = "ragmir.projectRegistry.v1"
const ACTIVE_PROJECT_KEY = "ragmir.activeProjectId.v1"
...
export interface RagmirProject {
  id: string
  name: string
  projectRoot: string
...
export function createProject(input: {
  ...
  return {
    ...
    rawDir: joinProjectPath(projectRoot, ".ragmir", "raw"),
    storageDir: joinProjectPath(projectRoot, ".ragmir", "storage"),
    filesIndexed: 0,
    chunksIndexed: 0,
    progress: 0,
    status: "needs-setup",
...
export function loadProjects(storage = browserStorage()): RagmirProject[] {
  if (!storage) {
    return []
  }
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter(isStoredRagmirProject).map(normalizeStoredProject)
      : []
  } catch {
    return []
  }
...
export function saveProjects(projects: RagmirProject[], storage = browserStorage()): void {
...
export function upsertProject(projects: RagmirProject[], project: RagmirProject): RagmirProject[] {
...
export function removeProject(projects: RagmirProject[], projectId: string): RagmirProject[] {
...
function projectNameFromRoot(projectRoot: string): string {
  ...
  return parts.at(-1) || "Ragmir project"
}
...
function normalizeStoredProject(project: StoredRagmirProject): RagmirProject {
  return {
    ...project,
...
function isStoredRagmirProject(value: unknown): value is StoredRagmirProject {
  if (!isRecord(value)) {
    return false
  }
  return (
