...
async function writeAgentMcpHelper(agent, input) {
    switch (agent) {
        ...
        case "kimi":
        case "cline":
            await writeFile(agent === "kimi" ? input.kimiConfigPath : input.clineConfigPath, `${JSON.stringify(mcpConfig(input.cwd, input.serveCommand, { [MIMIR_PROJECT_ROOT_ENV]: input.cwd }, input.mcpServerName), null, 2)}\n`, "utf8");
    }
}
