#!/usr/bin/env node
// Local library smoke for @jcode.labs/mimir.
//
...
// resolves "@jcode.labs/mimir" to THIS repo's local build (packages/mimir-core/dist),
...
// writes only to the sibling example's gitignored .mimir/storage.
...
async function main() {
  console.log("Running @jcode.labs/mimir from the local build against the synthetic corpus.")
