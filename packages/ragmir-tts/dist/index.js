...
export async function renderSpeech(options) {
    ...
    const modelPath = resolveFromCwd(cwd, options.modelPath ?? process.env.RAGMIR_TTS_MODEL_PATH ?? DEFAULT_TTS_MODEL_PATH);
    ...
    if (engine === "edge") {
        ...
        const voice = options.voice ?? process.env.RAGMIR_TTS_EDGE_VOICE ?? edgeVoiceForLanguage(language);
